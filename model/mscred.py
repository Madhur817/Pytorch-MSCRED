...
def attention(ConvLstm_out):
    ...
    return convLstmOut,attention_w
...
class Conv_LSTM(nn.Module):
    ...
    def forward(self, conv1_out, conv2_out, 
        ...
        conv1_lstm_out,attention_w1 = attention(conv1_lstm_out[0][0])
        conv2_lstm_out = self.conv2_lstm(conv2_out)
        conv2_lstm_out,attention_w2 = attention(conv2_lstm_out[0][0])
        conv3_lstm_out = self.conv3_lstm(conv3_out)
        conv3_lstm_out,attention_w3 = attention(conv3_lstm_out[0][0])
        conv4_lstm_out = self.conv4_lstm(conv4_out)
        conv4_lstm_out,attention_w4 = attention(conv4_lstm_out[0][0])

        return conv1_lstm_out.unsqueeze(0), conv2_lstm_out.unsqueeze(0), conv3_lstm_out.unsqueeze(0), conv4_lstm_out.unsqueeze(0),attention_w1,attention_w2,attention_w3,attention_w4
...
class MSCRED(nn.Module):
    ...
    def forward(self, x):
        ...
        conv1_lstm_out, conv2_lstm_out, conv3_lstm_out, conv4_lstm_out,attention_w1,attention_w2,attention_w3,attention_w4 = self.conv_lstm(
        ...
        # print()
        # print(gen_x.shape)
        return gen_x,attention_w1.flatten(),attention_w2.flatten(),attention_w3.flatten(),attention_w4.flatten()
