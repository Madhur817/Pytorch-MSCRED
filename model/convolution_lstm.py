...
class ConvLSTMCell(nn.Module):
	...
	def forward(self, x, h, c):
		ci = torch.sigmoid(self.Wxi(x) + self.Whi(h) + c * self.Wci.to(device))
		cf = torch.sigmoid(self.Wxf(x) + self.Whf(h) + c * self.Wcf.to(device))
		cc = cf * c + ci * torch.tanh(self.Wxc(x) + self.Whc(h))
		co = torch.sigmoid(self.Wxo(x) + self.Who(h) + cc * self.Wco.to(device))
		ch = co * torch.tanh(cc)
		return ch, cc
